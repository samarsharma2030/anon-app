...
const HomePage = () => {
  ...
  }, [location, user, isLoaded]);

  useEffect(() => {
    // Restore chat state on component mount
    const savedChannelId = localStorage.getItem('channelId');
    const savedIsChatConnected = localStorage.getItem('isChatConnected') === 'true'; // Convert string back to boolean
  
    if (savedChannelId && savedIsChatConnected) {
      setChannelId(savedChannelId);
      setIsChatConnected(savedIsChatConnected);
    }
  }, [isLoaded]);
  ...
  const onAcceptHandler = (channelId) => {
    ...
    localStorage.setItem('isChatConnected', 'true');
  };
      ...
      <div className="container mx-auto p-4 flex-grow">
        <div className="container mx-auto p-4 flex-grow">
          {nearbyUsers.length > 0 ? (
            <>
              <h2 className="text-lg font-bold px-4 py-2 border-b border-gray-200">Nearby Users</h2>
              <hr className='mb-4'/>
        ...
        {channelId && isChatConnected && (
          <div
            className={`fixed  bottom-0 right-0 w-full sm:w-96 bg-white border border-gray-300 sm:overflow-hidden mr-11 ${isChatMinimized ? "h-12" : 'h-[32rem]'}`}
          >
            <div className="flex justify-between items-center bg-blue-500 p-2 border-b border-gray-300">
              <h2 className="text-lg text-white font-semibold">
                Chat
              </h2>
              <button className="text-lg text-white font-semibold" onClick={toggleChatMinimized}>
                {isChatMinimized ? "Maximize" : "Minimize"}
              </button>
            </div>
            {!isChatMinimized && <ChatComponent 
